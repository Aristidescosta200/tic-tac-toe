...
export const useOnboarding = create<OnboardingStore & OnboardingActions>()((set) => ({
    ...INITIAL_ONBOARDING_STORE,
    onNextStep: () => set((state) => ({ currentStep: state.currentStep + 1 })),
    onPreviousStep: () => set((state) => ({ currentStep: state.currentStep - 1 })),
    setStepTitle: (title) => set(() => ({ stepTitle: title })),
    setStepDescription: (description) => set(() => ({ stepDescription: description })),
